...
const handleUpdateUser = async (e:React.FormEvent<HTMLFormElement>)=>{
  e.preventDefault();
  try{
    await axios.put(`${apiUrl}/api/${backendName}/users/${updateUser.id}`,{username:updateUser.username,email:updateUser.email})
 ...
 try {
  ...
  return (
    <div className={`user-interface ${bgColor} ${backendName} w-full max-w-md p-4 my-4 rounded shadow`}>
      <img src={`/flasklogo.svg`} alt={`${backendName}Logo`} className="w-20 h-20 mb-6 mx-auto"/>
      ...
      <form onSubmit={createUser} className="mb-6 p-4 bg-blue-100 rounded shadow">
       ...
       <form onSubmit={handleUpdateUser} className="mb-6 p-4 bg-blue-100 rounded shadow">
        ...
        className="mb-2 w-full p-2 border border-gray-300 rounded"
        />
        <button type="submit" className="w-full p-2 text-white bg-blue-500 rounded hover:bg-green-600">
          update User
        </button>
       </form>
      ...
      <div className="space-y-4">
        {users.map((user)=>
          <div key={user.id}className="flex items-center justify-between bg-white p-4">
            ...
            <button onClick={()=>deleteUser(user.id)}className={`${btnColor} text-white py-2 px-4 rounded hover:bg-red-600`}>
          Delete User
        </button>
          </div>
        )}
